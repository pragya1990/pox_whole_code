...
class VideoSlice (EventMixin):
    ...
    def _handle_PacketIn (self, event):
        ...
        def remove_firewall(event,packet,h1,h2,outputport):
            msg = of.ofp_flow_mod()
            msg.match = of.ofp_match()
            print h1
            print h2
            print outputport
            print of.OFPP_FLOOD
            print "Inside removing_firewall"
            msg.match.dl_src=EthAddr(h1)
            msg.match.dl_dst=EthAddr(h2)
        ...
        def forward (message = None):
            ...
            if Shared.FlowQoS == 1:
                print "SHARED QoS 1"
                for key,values in self.firewalldict.items():
                    print "Removing Firewall"
                    for k,v in values.items():
                        remove_firewall(event, packet, key,k, v)
                self.firewalldict = {}
                ...
                for line in lines:
                    ...
                    if words[0].strip() == "VideoRule":
                        ...
                        print h1
                        print h2
                        #install_firewall(event,packet,h1,h2)
                        
                        if h1 in self.firewalldict:
                            print "PRINTING FIREWALL DICT"
                            print self.firewalldict
                            self.firewalldict[h1][h2] = of.OFPP_NORMAL
                        else:
                            self.firewalldict[h1] = {}
                            self.firewalldict[h1][h2] = of.OFPP_NORMAL
                            print "PRINTING DICT"
                            print self.firewalldict
                            
                print "Printing Portmap", self.portmap
                for keys,values in self.portmap.items():
                    print "Inside kyvalues"
                    ...
                    if packet.dst.is_multicast:
                        ...
                        try:
                            ...
                            '''
                            for k1,v1 in self.firewalldict.items():
                                if k1 == this_dpid:
                                    for k2,v2 in v1.items():
                                        if k2 == next_dpid:
                                            self.firewalldict[k1][k2] = self.adjacency[this_dpid][next_dpid]
                                            print "SELF ADJ"
                                            print self.firewalldict[k1][k2]'''
                ...
                for key,values in self.firewalldict.items():
                    ...
                    for k,v in values.items():
                        install_firewall(event, packet, key, k)
                print "MAKE SHARED QoS 0"
                Shared.FlowQoS = 0
