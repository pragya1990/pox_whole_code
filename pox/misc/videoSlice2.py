...
import os
#import sys
#sys.path.append("/home/mininet/pox/pox/lib/FlowQoS.py")
#from pox.lib.FlowQos import FlowQoS
...
class VideoSlice (EventMixin):
	...
	'''self.portmap = {}
	fp = open('/home/mininet/pox/pox/misc/rules1.txt', 'r')
	lines = fp.readlines()
	fp.close()	
	counter = 0
	for line in lines:
    ...
    def _handle_PacketIn (self, event):
		...
		key_map = ()
		    ...
		    print "dstport: " + str(int(packet.find('tcp').dstport))
                    key_map = (str(this_dpid), packet.src, packet.dst, int(packet.find('tcp').dstport))
                    if not self.portmap.get(key_map):
			print "srcport: " + str(packet.find('tcp').srcport)
                        key_map = (str(this_dpid), packet.src, packet.dst, int(packet.find('tcp').srcport))
                        if not self.portmap.get(key_map):
